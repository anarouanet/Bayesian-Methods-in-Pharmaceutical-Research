plot_trajectories_AR_chapter<- function(riskProfObj,outFile,showRelativeRisk=F,orderBy=NULL,whichClusters=NULL,whichCovariates=NULL,
  ...
  if(yModel=='Longitudinal'){
    ...
    for(c in whichClusters){
      ...
      GPDF <- rbind(GPDF,data.frame("time"=tTimes*100+55,"mu"=params$mu+longMean,
    ...
    GPDF$line <- rep(c("solid","twodash","dotdash","longdash"),each=length(tTimes))
    rownames(GPDF)<-seq(1,nrow(GPDF),1)
    plotObj <- ggplot(GPDF)
    for(i in 1:nSubjects){
      df <- (data.frame(x=times[tMat[i,1]:tMat[i,2]]*100+55,y=yData[tMat[i,1]:tMat[i,2]]+longMean))
      plotObj <- plotObj + geom_line(data=df,aes(x,y), alpha=0.3,colour='azure4')
    }
    plotObj <- plotObj + 
      geom_line(aes(x=time, y=mu, group=cluster, colour=as.factor(cluster), linetype=as.factor(cluster)), size=2.5)+
      scale_linetype_manual(values=c("solid","dotted","dotdash","longdash"))
    #plotObj <- plotObj + geom_line(aes(x=time,y=mu,group=cluster,colour=as.factor(cluster)),size=2.5)+scale_linetype_manual(values=c("solid","twodash","dotdash","longdash"))
    #plotObj <- plotObj + geom_line(aes(x=time,y=mu+sigma,group=cluster,colour=as.factor(cluster)))
    #plotObj <- plotObj + geom_line(aes(x=time,y=mu-sigma,group=cluster,colour=as.factor(cluster)))
    plotObj <- plotObj + labs(col="Cluster", linetype="Cluster")
    plotObj <- plotObj + labs(y="Normalised MMSE\n")+theme(axis.title.y=element_text(size=30,angle=90))
    plotObj <- plotObj + labs(x="\nAge")+theme(axis.title.x=element_text(size=30))
    plotObj <- plotObj + coord_cartesian(ylim = c(0,103))#c(min(GPDF$mu-GPDF$sigma),max(GPDF$mu+GPDF$sigma)))
    plotObj <- plotObj + theme(axis.text.x=element_text(size=20)) + theme(axis.text.y=element_text(size=20))
    plotObj <- plotObj + theme(axis.line.x = element_line(colour="black",size=2),
                               axis.line.y = element_line(colour="black",size=2))
    plotObj <- plotObj + theme(legend.title=element_text(size=20),legend.text=element_text(size=15))
    plotObj <- plotObj + theme(plot.title=element_text(size=20), legend.key.width = unit(2,"cm") )
    ...
    plotObj <- plotObj + labs(x="\nAge")+theme(axis.title.x=element_text(size=40))
