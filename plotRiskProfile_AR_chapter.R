plotRiskProfile_AR_chapter<-function(riskProfObj,outFile,showRelativeRisk=F,orderBy=NULL,whichClusters=NULL,whichCovariates=NULL,
  ...
  pdf(outFile,width=16,height=4.50)
  ...
  if(includeResponse){
    if(yModel=='Categorical'){
      ...
      plotObj<-plotObj+labs(y="Probability")+theme(axis.title.y=element_text(size=10,angle=90)) + ylim(0,1)
    ...
    if(xModel=='Discrete'){
      ...
      if (j<=nDiscreteCovs){
        ...
        plotObj<-plotObj+
          #scale_fill_manual(values = c(high ="#CC0033",low ="#0066CC", avg ="#33CC66"))+
          #scale_colour_manual(values = c(high ="#CC0033",low ="#0066CC", avg ="#33CC66"))+
          theme(legend.position="none")+labs(x="Cluster")+theme(axis.title.x=element_text(size=15), axis.text=element_text(size=15))
        if(j==1){
          plotObj<-plotObj+labs(y="Probability")+theme(axis.title.y=element_text(size=20,angle=90), axis.text=element_text(size=15))
        }else{
          plotObj<-plotObj+theme(axis.title.y=element_blank())
        }
        plotObj<-plotObj+labs(title=covNames[j],plot.title=element_text(size=15)) + ylim(0,1)
        ...
        plotObj<-plotObj+
          #scale_fill_manual(values = c(high ="#CC0033",low ="#0066CC", avg ="#33CC66"))+
          #scale_colour_manual(values = c(high ="#CC0033",low ="#0066CC", avg ="#33CC66"))+
          theme(legend.position="none")+labs(x="Cluster")+theme(axis.title.x=element_text(size=15),
        ...
        plotObj<-plotObj+labs(title=covNames[j],plot.title=element_text(size=15)) + ylim(-1,0.5)
        plotObj<-plotObj+
  ...
  if(yModel=='MVN'){
    ...
    pdf(paste(strsplit(outFile,"\\.")[[1]][1],'-MVN.pdf',sep=""),width=8,height=4)
    ...
    for(j in 1:nOutcomes){
      ...
      plotObj<-plotObj+
        #scale_fill_manual(values = c(high ="#CC0033",low ="#0066CC", avg ="#33CC66"))+
        #scale_colour_manual(values = c(high ="#CC0033",low ="#0066CC", avg ="#33CC66"))+
        theme(legend.position="none")+labs(x="Cluster")+theme(axis.title.x=element_text(size=20), axis.text=element_text(size=15))
      if(j==1){
        plotObj<-plotObj+labs(y="Mean")+theme(axis.title.y=element_text(size=20,angle=90))
      }else{
        plotObj<-plotObj+theme(axis.title.y=element_blank())
